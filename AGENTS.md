...
- Google backup must use `drive.appdata`/`appDataFolder`; no alternative cloud-storage provider is part of the current product contract.
...
- The Gmail OAuth callback must enqueue an initial sync automatically.
