...
type RelayPayload = {
  ...
  receivedAt?: unknown;
  rawSha256?: unknown;
  rawMimeBase64?: unknown;
  authentication?: unknown;
  forwardingProviderHint?: unknown;
};
type RelayAliasRow = {
  alias_id: string;
  user_id: string;
...
Deno.serve(async (request) => {
  try {
    ...
    const { data: claimed, error: claimError } = await service.rpc(
      "service_claim_email_relay_replay",
      { p_nonce_hash: nonceHash, p_expires_at: expiresAt },
    );
    if (claimError) throw claimError;
    if (!claimed) throw new HttpError(409, "relay_replay_detected");

    let body: RelayPayload;
    try {
      body = JSON.parse(rawBody) as RelayPayload;
    } catch {
      throw new HttpError(400, "invalid_json");
    }
    const recipient = sanitizeRelayHeader(body.recipient, 320);
    if (!recipient) throw new HttpError(422, "missing_recipient");
    const domain = requiredEnv("CAPITALFLOW_EMAIL_RELAY_DOMAIN");
    const rawToken = extractAliasToken(recipient, domain);
    const tokenHash = await sha256Base64Url(rawToken);
    const { data: aliasData, error: aliasError } = await service
      .rpc("service_resolve_email_relay_alias", { p_token_hash: tokenHash })
      .maybeSingle();
    ...
    const sourcePayload = {
      ...
      sender_normalized: verification
      ...
      recipient_alias_id: alias.alias_id,
      recipient_source_id: sourceId,
      forwarding_provider_hint: sourceProvider,
      message_id: messageId,
      received_at: receivedAt,
      raw_sha256: computedHash,
    };
    ...
    if (sourceMatch?.match_status === "revoked" && !verification) {
      await service
        ...
        .update({
          ...
          metadata: { ...metadata, event_type: "relay_source_revoked" },
        })
        .eq("id", source.id);
    ...
    if (verification) {
      ...
      const { error: inboxError } = await service.rpc(
        "service_create_email_relay_forwarding_verification",
        {
          p_user_id: alias.user_id,
          p_alias_id: alias.alias_id,
          p_source_id: sourceId,
      ...
      return json(
        { accepted: true, verification: "forwarding", financial: false },
        202,
      );
    }
    ...
    const parsed = await parseMailMessage({
      provider: "email_relay",
      externalId: messageId ?? fingerprint,
      occurredAt,
      sender: from ?? envelopeSender,
      title: subject,
      text,
    ...
    const result = await ingestCandidate(
      ...
      parsed,
      alias.connection_id,
      { aliasId: alias.alias_id, sourceId, providerHint: sourceProvider },
    );
    await service
      .from("source_events")
      .update({
        ...
        parser_rule_version: parsed.parserVersion,
        detected_amount_minor: parsed.amountMinor,
        detected_currency: parsed.currency,
        detected_direction: parsed.proposedKind,
        detected_merchant: parsed.merchant,
        detected_confidence: parsed.confidence,
        candidate_id: result.candidateId,
        transaction_id: result.automation?.transactionId ?? null,
        processing_status: "parsed",
        metadata: {
          ...metadata,
          parser_version: parsed.parserVersion,
          automation_outcome: result.automation?.outcome ?? null,
        },
      })
      .eq("id", source.id);
    ...
    return json(
      {
        ...
        candidateCreated: result.inserted,
        automation: result.automation?.outcome ?? null,
      },
      202,
    );
  } catch (error) {
