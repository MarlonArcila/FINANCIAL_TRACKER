...
function decodeQuotedPrintable(value: string): string {
  ...
  for (let i = 0; i < source.length; i += 1) {
    ...
    const encoded = encoder.encode(source[i]);
...
function decodePart(body: string, encoding: string | null): string {
  ...
  if (enc.includes("base64")) {
    try {
      const binary = atob(body.replace(/\s+/gu, ""));
...
export function extractTextFromMime(raw: string): string {
  ...
  if (boundary) {
    ...
    for (const part of parts) {
      const ps = /\r?\n\r?\n/u.exec(part);
      if (!ps || ps.index === undefined) continue;
      const ph = part.slice(0, ps.index);
      const pb = part.slice(ps.index + ps[0].length);
      const ct =
        /^content-type:\s*([^\r\n]+)/imu.exec(ph)?.[1]?.toLowerCase() ??
        "text/plain";
    ...
    return (texts.length ? texts : htmls)
      .join("\n\n")
      .replace(/\u0000/gu, "")
      .slice(0, 100_000)
      .trim();
...
export type ForwardingVerificationDetection = {
  provider: "gmail" | "outlook" | "proton" | "other";
  url: string | null;
  code: string | null;
  excerpt: string | null;
  confidence: number;
...
export function detectForwardingVerification(
  ...
  if (
    !/(gmail.{0,30}forwarding|forwarding.{0,30}confirmation|confirmaci[oó]n.{0,30}reenv[ií]o|reenv[ií]o.{0,30}gmail)/iu.test(
      combined,
    )
  )
    return null;
  ...
  if (!safeUrl && !codeMatch) return null;
  return {
    provider: "gmail",
    url: safeUrl,
    code: codeMatch?.[1] ?? null,
    excerpt: sanitizeRelayHeader(subject ?? text, 240),
    confidence: 0.95,
  };
}
...
export function detectGmailForwardingConfirmation(
  ...
  const detection = detectForwardingVerification(subject, text);
  return detection ? { url: detection.url, code: detection.code } : null;
}
