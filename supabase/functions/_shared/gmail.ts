...
export async function exchangeGoogleCode(code: string, codeVerifier: string): Promise<OAuthTokenSet> {
  ...
  if (!response.ok || typeof payload.access_token !== "string") throw new Error(`Google token exchange failed: ${JSON.stringify(payload)}`);
  return {
...
export async function getGoogleAccessToken(service: SupabaseClient, connectionId: string): Promise<string> {
  ...
  const payload = await response.json() as Record<string, unknown>;
  if (!response.ok || typeof payload.access_token !== "string") throw new Error(`Google token refresh failed: ${JSON.stringify(payload)}`);
...
export async function syncGmailConnection(
  ...
  if (connection.cursor && !needsInitialBackfill) {
    try {
      ...
      console.warn("Gmail history fallback", error);
  ...
  if (!connection.cursor || needsInitialBackfill || historyUnavailable) {
    ...
    listUrl.searchParams.set("q", optionalEnv("GMAIL_QUERY") ?? "newer_than:30d {compra pago abono transferencia charged received}");
